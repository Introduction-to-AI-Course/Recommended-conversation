...
def get_data_idx(utt, type, entity, bot, word_dict, type_dict, entity_dict, bot_dict):
    utt_idx = text_generator(word_dict, utt)#整个对话的向量表达
    type_idx = list()
    entity_idx = list()
    bot_idx = list()
...
def get_word_dict(utterances):
    ...
    word_set = set()
    for utterance in utterances:#对每段对话
        for u in utterance:#对每段对话里的每一句
            u = re.sub("\[\d*\]", "", u)
            u = re.sub(r'[~`!#$%^&*()_+-=|\';":/.,?><~·！@#￥%……&*（）——+-=“：’；、。，？》《{}]+', "", u)
            for word in u.strip().split(" "):
                if word is not "":#每一个分词
                    word_set.add(word)
...
def get_test_data(word_dict, goal_type_dict, goal_entity_dict):
    ...
    with open("../origin_data/test.txt", "r") as f:
        for idx, line in enumerate(f.readlines()):
            ...
            #抽取每段话里的用户回答通过之前构造完的词典进行映射，转换为词向量
